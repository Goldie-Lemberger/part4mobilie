...
class Distance_TFL:
    ...
    def run(self, path, prev_frame, tfls,tfl_points, currentEm,focal,pp):
        ...
        prev_container = Frame_Model(path)
        curr_container = Frame_Model(prev_frame)
        prev_container.traffic_light = np.array(tfls)
        curr_container.traffic_light = np.array(tfl_points)
